...
function NotepadInner({ userId, today, initialNote }: Props) {
  ...
  // The not-yet-persisted edit. Held in a ref (not just the debounce timer) so
  // navigation and unmount can flush it instead of dropping the keystrokes.
  const pending = useRef<{ date: string; content: string } | null>(null);
  // Monotonic token so a slow day-load can't clobber a newer one.
  const loadSeq = useRef(0);

  // Write whatever edit is still waiting (debounce not yet fired) right now.
  ...
  // Flush on unmount so closing/leaving inside the debounce window can't lose
  // the last keystrokes. Unmount alone doesn't cover closing the tab/app —
  // React never tears down — so also flush on pagehide (the reliable
  // end-of-session signal, including iOS PWA backgrounding).
  useEffect(() => {
  ...
  async function goTo(nextDate: string) {
    ...
    const { data } = await supabase
      ...
      .eq("date", nextDate)
      .maybeSingle();
    // A faster, more recent navigation already landed — drop this result.
  ...
  function onChange(value: string) {
    setContent(value);
    setSaveState("saving");
    pending.current = { date, content: value };
  ...
  return (
    <section className="card flex flex-col rounded-2xl bg-tint px-5 py-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {!isToday && (
            <button
              onClick={() => goTo(today)}
              className="min-h-11 px-1 text-xs text-muted underline-offset-2 hover:underline"
            >
              Today
            </button>
          )}
      ...
      <textarea
        ...
        className="mt-3 min-h-28 flex-1 resize-none rounded-lg bg-bg px-4 py-3 text-base leading-7 outline-none placeholder:text-muted focus:ring-2 focus:ring-ring sm:text-sm lg:min-h-40"
      />
    </section>
  );
}
