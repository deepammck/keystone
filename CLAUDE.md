...
- ICS export: `lib/ics.ts` (pure RFC 5545 generator, no deps) powers the EventList "Export" button (client-side blob download; works in local + Supabase mode) and `app/api/calendar/route.ts` (session-authenticated download in Supabase mode; 404 in local mode).
- Goals: `goals` table (`009_goals.sql`: `id/user_id/title/completed/created_at`) driven by the generic `useCollection` hook (no bespoke hook); rendered by `components/GoalBanner.tsx` as always-visible aspiration pills in the Header's middle dead-band (between the progress chips and the settings gear; wraps to its own row on mobile) — NOT a checklist and NOT a seventh section. Goals here are read, not ticked off: there is no checkbox and no completed toggle, so the `completed` column is retained but unused; add a goal or remove it with the ×. Wired via `Dashboard` (`useCollection<Goal>("goals", …)` → `Header` props `goals`/`onAddGoal`/`onDeleteGoal`) + `initialGoals` from `app/dashboard/page.tsx` / `loadLocalInitial`.
...
- Supabase mode: set `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY` in `.env.local`, then apply migrations `001` → `011` in order.
